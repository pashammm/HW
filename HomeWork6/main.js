...
function countLetterA(str) {
  // let newArr = [];
  let count = str.split('').filter(function(val){
    return val === 'a' && 'A'; 
  })
  console.log(count.length)
};
countLetterA(user.name);
 ...
 *
 * */
let sentance = `You don't have to do anything special to begin using the DOM. Different browsers have different implementations of the DOM`

function reverseEachWord1(string) {
  let newArr = string.split(' ');
  let ArrAfter = [];
  let revWord = newArr.forEach(function(element) {
    ArrAfter.push(element.split('').reverse().join(''));
  })
  console.log(ArrAfter.join(' '))
};
reverseEachWord1(sentance);



//Task 2.2
function reverseEachWord(string,arg) {
 let newArr = string.split(' ');
  let ArrAfter = [];
  let revWord = newArr.forEach(function(element) {
    ArrAfter.push(element.split('').reverse().join(''));
  });
  let endSentanse = ArrAfter.join(' ');
  if(arg == true){
    console.log(endSentanse)
  } else if (arg == false){
    console.log(endSentanse.split(' ').reverse().join(' '));
...
let word = 'Both - Java - and - Java - Script - is - programming - and - programming - OOPBased';
...
function createHashTags(arr) {
  ...
  arr.forEach(function(element) {
    obj[element._id] = element.company;
  }, this);
